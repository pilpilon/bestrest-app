...
## 1. Context & Overview
**BestRest** is a SaaS application tailored for the Israeli restaurant and catering industry. Its primary goal is to provide kitchen and restaurant managers with tools to manage expenses, analyze food costs, and monitor market pricing. The application is built with a "Hebrew First UX" and features a responsive, mobile-first design leveraging modern UI paradigms (e.g. glassmorphism, bold high-contrast dark mode aesthetics).

## 2. Toolchain & Tech Stack
- **Frontend Framework**: React 19 + Vite + TypeScript
- **Styling**: Tailwind CSS v4 (PostCSS)
- **State Management & Data**: Custom React Hooks interfacing with Firebase
- **Backend Infrastructure**: Vercel Serverless Functions (`/api/*`)
- **Database & Auth**: Firebase Authentication & Firestore (Client-side usage & Admin SDK on Vercel)
...
The application uses a multi-tenant structure organized around `businessId`:

- **`users/{userId}`** 
  - Represents the user profiles.
  - Fields: `role` (e.g., 'admin', 'manager', 'accountant'), `businessId`, `subscriptionTier`, `ocrScanResetDate`.
  - Rules: Users can update their own non-critical fields. Admins have full access.
...
- **`expenses/{expenseId}`**
  ...
  - Rules: Read/Write restricted to authenticated users matching the `businessId` with appropriate roles ('admin', 'manager', 'accountant').

- **`market_price_cache/{itemName}`**
  - Caches AI-generated market insights to reduce API usage.
  - Fields: `itemName`, `marketPrice`, `recommendedUnit`, `confidence` (high|medium|low), `cachedAt`.
  - Cache TTL is 7 days.

*(Note: Legacy collections like `staff`, `availability`, `published_schedules`, and `negotiation_logs` are being migrated to subcollections under `businesses/{businessId}/`)*

## 4. AI Implementation & Prompt Configuration
The core logic relies on extracting structured data from right-to-left (RTL) Hebrew receipts, menus, and invoices, as well as providing culinary and market estimations.

### 4.1. Invoice & Receipt Processing (`ocr.ts`, `ocr-vision-items.ts`, `ocr-items.ts`)
- **Document AI / Vision API**: Used to extract structured header entities (supplier name, total amount, invoice date) and raw text.
...
### 4.2. Market Pricing Insights (`market-insights.ts`)
...
### 4.3. Kitchen & Food Cost Prediction (`predict-cost.ts`)
...
### 4.4. Menu OCR Parsing (`ocr-menu.ts`)
...
## 5. Known Edges & Defensive Mechanisms
